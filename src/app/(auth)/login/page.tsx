...
import { Sparkles, ArrowLeft, Eye, EyeOff, Shield, Briefcase, Code2 } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      toast.success("Welcome back!", "Signed in successfully to NexusHR.");
      router.push("/dashboard");
    }, 500);
  };

  const handleOAuthLogin = (provider: string) => {
    toast.info(`${provider} Auth`, `Connecting with ${provider}...`);
    setIsLoading(true);
    setTimeout(() => {
      setIsLoading(false);
      toast.success("Signed in!", `Successfully authenticated with ${provider}.`);
      router.push("/dashboard");
    }, 600);
  };

  const handleDemoLogin = (roleEmail: string, roleName: string) => {
    setEmail(roleEmail);
    setPassword("password123");
    setIsLoading(true);
    toast.success("Welcome back!", `Signed in as ${roleName}. Opening dashboard...`);
    setTimeout(() => {
      setIsLoading(false);
      router.push("/dashboard");
    }, 400);
    ...
    <div className="min-h-screen w-full flex flex-col lg:flex-row bg-background text-foreground">
      ...
      <div className="flex-1 min-h-screen flex flex-col justify-between p-4 sm:p-8 lg:p-10 relative overflow-y-auto">
        ...
        <div className="w-full max-w-[400px] mx-auto my-auto py-2">
          ...
          {/* 2x2 OAuth Social / SSO Buttons */}
          <div className="grid grid-cols-2 gap-2.5 mb-5">
            {/* Google */}
            <Link
              href="/auth/google"
              className="flex items-center justify-center gap-2.5 h-10 px-4 rounded-lg border border-border/80 dark:border-zinc-800 bg-card hover:bg-muted/40 hover:border-border text-xs sm:text-[13px] font-medium text-foreground transition-all shadow-2xs group cursor-pointer"
            >
            ...
            <Link
              ...
              className="flex items-center justify-center gap-2.5 h-10 px-4 rounded-lg border border-border/80 dark:border-zinc-800 bg-card hover:bg-muted/40 hover:border-border text-xs sm:text-[13px] font-medium text-foreground transition-all shadow-2xs group cursor-pointer"
            >
              <svg className="h-4 w-4 fill-primary shrink-0" viewBox="0 0 24 24">
          ...
          <div className="mt-4 rounded-xl bg-muted/30 border border-border/80 p-2.5 text-xs">
            <p className="font-semibold text-foreground flex items-center gap-1.5 mb-2 text-[11px]">
              <Shield className="h-3.5 w-3.5 text-primary" /> Instant Demo Accounts
            </p>
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => handleDemoLogin("sarah.jenkins@company.com", "HR Admin")}
                className="flex items-center gap-2 rounded-lg bg-card px-3 py-1.5 text-xs font-medium border border-border/80 hover:border-primary text-foreground text-left transition-colors cursor-pointer group"
              >
              ...
              <button
                type="button"
                onClick={() => handleDemoLogin("alexander.wright@company.com", "VP Engineering")}
                className="flex items-center gap-2 rounded-lg bg-card px-3 py-1.5 text-xs font-medium border border-border/80 hover:border-primary text-foreground text-left transition-colors cursor-pointer group"
              >
                <Code2 className="h-3.5 w-3.5 text-primary shrink-0" />
                <span className="truncate">VP Engineering</span>
              </button>
            </div>
          </div>
        </div>
